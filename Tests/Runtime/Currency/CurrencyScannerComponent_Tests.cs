...
namespace Tests.MartonioJunior.Trinkets.Currency {
  public class CurrencyScannerComponent_Tests
      : ComponentTestModel<CurrencyScannerComponent> {
#region Constants
...
#region Method Tests
    ...
    [Test]
    public void
    Check_ReturnsTrueWhenWalletHasEnoughResourcesAndComponentIsEnabled() {
    ...
    public void Scan_FiresScanEventWhenExecuted() {
      ...
      InsertMoneyOnWallet(AmountOnWallet);
      modelReference.onScanWallet += (walletHasEnough) => scanWasSuccessful =
          walletHasEnough;
      modelReference.Scan(Wallet);
